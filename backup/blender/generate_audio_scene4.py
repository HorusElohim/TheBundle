...
def build_audio_grid_group(name: str = "AudioGridRipple") -> bpy.types.NodeTree:
    ...
    # Ripple controls
    s_freq  = ng.interface.new_socket("Frequency", in_out="INPUT", socket_type='NodeSocketFloat'); s_freq.default_value = 3.0
    s_speed = ng.interface.new_socket("Speed", in_out="INPUT", socket_type='NodeSocketFloat');    s_speed.default_value = 1.0
    s_amp   = ng.interface.new_socket("Amplitude", in_out="INPUT", socket_type='NodeSocketFloat'); s_amp.default_value = 1.0

    # Nodes
    n_in   = ng.nodes.new("NodeGroupInput");       n_in.location = (-1200, 0)
    n_out  = ng.nodes.new("NodeGroupOutput");      n_out.location = (  600, 0)

    n_grid = ng.nodes.new("GeometryNodeMeshGrid"); n_grid.location = (-1000, 0)
    n_pts  = ng.nodes.new("GeometryNodeMeshToPoints"); n_pts.location = (-800, 0); n_pts.mode = 'VERTICES'

    n_pos  = ng.nodes.new("GeometryNodeInputPosition"); n_pos.location = (-1000, -250)
    n_sep  = ng.nodes.new("ShaderNodeSeparateXYZ");     n_sep.location = (-800, -250)
    n_combxy = ng.nodes.new("ShaderNodeCombineXYZ");    n_combxy.location = (-600, -250); n_combxy.inputs["Z"].default_value = 0.0
    n_len  = ng.nodes.new("ShaderNodeVectorMath");      n_len.location = (-400, -250); n_len.operation = 'LENGTH'

    n_time = ng.nodes.new("GeometryNodeInputSceneTime"); n_time.location = (-1000, -500)

    n_mul_freq = ng.nodes.new("ShaderNodeMath"); n_mul_freq.location = (-200, -250); n_mul_freq.operation = 'MULTIPLY'
    n_mul_spd  = ng.nodes.new("ShaderNodeMath"); n_mul_spd.location = (-200, -500); n_mul_spd.operation = 'MULTIPLY'
    n_sub      = ng.nodes.new("ShaderNodeMath"); n_sub.location = (   0, -300); n_sub.operation = 'SUBTRACT'
    n_sin      = ng.nodes.new("ShaderNodeMath"); n_sin.location = ( 200, -300); n_sin.operation = 'SINE'

    n_mul_amp  = ng.nodes.new("ShaderNodeMath"); n_mul_amp.location = ( 400, -300); n_mul_amp.operation = 'MULTIPLY'
    n_mul_audio= ng.nodes.new("ShaderNodeMath"); n_mul_audio.location = ( 600, -300); n_mul_audio.operation = 'MULTIPLY'

    n_combz    = ng.nodes.new("ShaderNodeCombineXYZ"); n_combz.location = (800, -200)
    n_set      = ng.nodes.new("GeometryNodeSetPosition"); n_set.location = (1000, 0)

    n_ico   = ng.nodes.new("GeometryNodeMeshIcoSphere"); n_ico.location = (-600, 200)
    n_inst  = ng.nodes.new("GeometryNodeInstanceOnPoints"); n_inst.location = (1200, 0)
    n_real  = ng.nodes.new("GeometryNodeRealizeInstances"); n_real.location = (1400, 0)
...
def main():
    try:
        ...
        # Save
        bpy.ops.wm.save_as_mainfile(filepath="audio_scene4.blend")
        log.info("💾 Saved Blender project as audio_scene4.blend")
