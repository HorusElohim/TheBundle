...
from . import CORE_LOGGER, logger
...
class Data(BaseModel):
    ...
    def from_dict(cls: Type[D], data: dict) -> D:
        ...
        try:
            instance = cls(**data)
            CORE_LOGGER.debug(f"{logger.Emoji.success}")
        except Exception as ex:
            CORE_LOGGER.error(f"{logger.Emoji.failed} {ex}")
            raise ex
        return instance
    ...
    @classmethod
    def _from_json_path(cls: Type[D], json_path: Path) -> D:
        """
        ...
        """
        json_str = json_path.open("rt").read()
        return cls._from_json_str(json_str)

    @classmethod
    def _from_json_str(cls: Type[D], json_str: str) -> D:
        """
    ...
    @classmethod
    def from_json(cls: Type[D], json_source: str | Path) -> D:
        """
        ...
        try:
            match json_source:
                case str():
                    return cls._from_json_str(json_source)
                case Path():
                    return cls._from_json_path(json_source)
                case _:
                    raise RuntimeError(f"Unsupported {json_source=}")
        except Exception as ex:
            CORE_LOGGER.error(f"{logger.Emoji.failed} {ex}")
            raise ex
    ...
    def dump_json(self, path: Path) -> None:
        ...
        """
        try:
            path.open("wt").write(self.as_json())
            CORE_LOGGER.debug(f"{logger.Emoji.success}")
        except Exception as ex:
            CORE_LOGGER.error(f"{logger.Emoji.failed} {ex}")
            raise ex

    def dump_jsonschema(self, path: Path, mode: json_schema.JsonSchemaMode = "validation") -> None:
        """
        Write the JSON Schema of the model to a file.

        Args:
            path (Path): The file path where the JSON Schema will be saved.
            mode (json_schema.JsonSchemaMode): The mode of the JSON Schema, defaults to "validation".

        Raises:
            Exception: If writing the JSON Schema to the file fails.
        """
        try:
            path.open("wt").write(self.as_jsonschema_str(mode))
            CORE_LOGGER.debug(f"{logger.Emoji.success}")
        except Exception as ex:
            CORE_LOGGER.error(f"{logger.Emoji.failed} {ex}")
            raise ex
    ...
    @classmethod
    def as_jsonschema(cls, mode: json_schema.JsonSchemaMode = "validation") -> dict:
        """
        ...
        """
        try:
            json_str = cls.model_json_schema(mode=mode)
            CORE_LOGGER.debug(f"{logger.Emoji.success}")
        except Exception as ex:
            CORE_LOGGER.error(f"{logger.Emoji.failed} {ex}")
            raise ex
    ...
    @classmethod
    def as_jsonschema_str(cls, mode: json_schema.JsonSchemaMode = "validation") -> str:
        """
        ...
        """
        try:
            json_str = json.dumps(cls.as_jsonschema(mode), indent=4)
            CORE_LOGGER.debug(f"{logger.Emoji.success}")
        except Exception as ex:
            CORE_LOGGER.error(f"{logger.Emoji.failed} {ex}")
            raise ex
