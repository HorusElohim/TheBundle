...
class BlenderRuntime:
    ...
    async def discover_default(self) -> BlenderEnvironment:
        ...
        raise FileNotFoundError(
            "Unable to locate a Blender installation automatically. Set BUNDLE_BLENDER_PYTHON or run `bundle blender download`."
        )
...
def _version_dir(path: Path) -> Path | None:
    ...
    if not dirs and (path / "Resources").is_dir():
        dirs = [item for item in (path / "Resources").iterdir() if item.is_dir() and _VERSION_RE.match(item.name)]
