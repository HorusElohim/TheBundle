...
def cprofile(
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwds):
            # Enable profiling
            TEST_LOGGER.debug(f"profiling {func.__name__} ...")
            pr = cProfile.Profile()
            pr.enable()
            ...
            TEST_LOGGER.debug(f"{func.__name__}= executed in {utils.format_duration_ns(elapsed_ns)}")
            ...
            if cprofile_dump_dir:
                ...
                TEST_LOGGER.debug(f"Dumping cProfile stats to: {dump_file}")
