...
function notifyParent(state, detail, progress) {
...
try {
  Promise.all([getUsdModule({
    ...
    notifyParent("module_ready", "USD module ready", 0.6);
    animate();
    if (filename) {
      console.log("Loading File...");
      if (messageLog) messageLog.textContent = "Loading File " + filename;
      notifyParent("file_loading", "Loading scene in viewer…", 0.75);
...
async function loadUsdFile(directory, filename, path, isRootFile = true) {
  ...
  ready = true;
  notifyParent("loaded", "Scene ready", 1);
  ...
  for(let link of fileLoadingLinks) {
    link.addEventListener('click', async function(event) {
      ...
      if (filename) {
        ...
        notifyParent("file_loading", "Downloading file…", 0.7);
        updateUrl();
