...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers",
        "bundle_tracer_sync(log_level=None, exc_log_level=None): Apply trace sync decorator with specified parameters",
    )
    config.addinivalue_line(
        "markers",
        "bundle_tracer_async(log_level=None, exc_log_level=None): Apply trace async decorator with specified parameters",
    )
...
def pytest_collection_modifyitems(session, config, items):
    ...
    for item in items:
        # Bundle tracer marker Sync
        if trace_marker_sync := item.get_closest_marker("bundle_tracer_sync"):
            log.testing(f"added @tracer.Sync.call_raise marker to {item.name}")
            trace_log_level = trace_marker_sync.kwargs.get("log_level", bundle.core.logger.Level.TESTING)
            trace_exc_log_level = trace_marker_sync.kwargs.get("exc_log_level", bundle.core.logger.Level.TESTING)

            # Apply the cprofile_decorator with parameters, excluding cprofile_folder
            item.obj = bundle.core.tracer.Sync.decorator.call_raise(
                log_level=trace_log_level, exc_log_level=trace_exc_log_level
            )(item.obj)

        # Bundle tracer marker Async
        if trace_marker_async := item.get_closest_marker("bundle_tracer_async"):
            log.testing(f"added @tracer.Async.call_raise marker to {item.name}")
            trace_log_level = trace_marker_async.kwargs.get("log_level", bundle.core.logger.Level.TESTING)
            trace_exc_log_level = trace_marker_async.kwargs.get("exc_log_level", bundle.core.logger.Level.TESTING)

            # Apply the cprofile_decorator with parameters, excluding cprofile_folder
            item.obj = bundle.core.tracer.Async.decorator.call_raise(
                log_level=trace_log_level, exc_log_level=trace_exc_log_level
            )(item.obj)
