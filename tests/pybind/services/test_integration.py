...
async def test_cmake_and_pkgconfig_integration(built_example_module, request):
    ...
    pc_file = cpp_module_path.pkgconfig / "example_module.pc"
    ...
    spec = PkgConfigSpec(packages=["example_module"], extra_dirs=[str(cpp_module_path.pkgconfig.as_posix())])
    resolved = await pkg_service.resolve(spec)
    # Clean extra_dirs for stable reference comparison
    resolved.spec.extra_dirs = [str(Path(d).relative_to(cpp_module_path.install)) for d in resolved.spec.extra_dirs]
    for pkg_resolved in resolved.resolved:
        pkg_resolved.include_dirs = [str(Path(d).relative_to(cpp_module_path.install)) for d in pkg_resolved.include_dirs]
        pkg_resolved.library_dirs = [str(Path(d).relative_to(cpp_module_path.install)) for d in pkg_resolved.library_dirs]
