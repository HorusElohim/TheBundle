...
async def test_cmake_configure(get_tmp_example_module: Path):
    ...
    ).is_file(), "CMakeCache.txt not found in build directory"
    ...
    assert (
        f"CMAKE_INSTALL_PREFIX:PATH={install_prefix.resolve().as_posix()}"
        in cache_content
    )
...
async def test_cmake_build_and_install(get_tmp_example_module: Path):
    ...
    assert any(
        (source_dir / build_dir_name).iterdir()
    ), "Build directory is empty after default build"
